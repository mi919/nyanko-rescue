...
| 状態 | 不透明度 | 影 | 視覚効果 |
|------|--------|----|---------|
| 未開封 | 不透明（ソリッド） | 外凸（NEU_REST_SHADOW） | 「叩ける蓋」 |
| 押下中 | 不透明 | 内凹（NEU_PRESSED_SHADOW） | 「押し込まれた」 |
| 開封済み 空マス | `rgba(255,255,255,0.42)` | 強い内凹 | 「蓋が外れて背景が透けて見える」 |
| 開封済み 数字マス | `rgba(255,253,248,0.72) → rgba(248,244,234,0.78)` | 強い内凹（茶系） | 「クリーム色の紙が見える」、数字を読みやすく |
| 開封済み 猫マス | `rgba(232,245,233,0.78) → rgba(214,234,216,0.82)` | 強い内凹 + ほのかなグロー | パステル緑が透ける |
| 開封済み 犬マス | `rgba(255,227,227,0.78) → rgba(251,207,207,0.82)` | 強い内凹 + ほのかなグロー | パステル赤が透ける |

数字テキストは `font-weight: 900` ＋ `text-shadow: 0 1px 0 rgba(255,255,255,0.7)` で抜き処理を入れて、半透明背景でも視認性を確保する。
