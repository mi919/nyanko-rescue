...
| No. | 定数名 | 用途 | 形式 | 解像度 | base64サイズ |
|-----|--------|------|------|--------|-------------|
| 1 | `SPRITE_URL` | キャラクタースプライトシート | PNG（透過） | 256×192px | ~105KB |
| 2 | `BG_URL` | タイトル画面メインビジュアル | JPEG | 600×555px | ~96KB |
| 3 | `LOGO_URL` | タイトルロゴ | PNG（透過） | 可変 | ~66KB |
| 4 | `DOG_BARK_URL` | 犬噛みつき演出用 | PNG（透過） | 276×280px | ~84KB |
...
|------|-----|
| 形式 | JPEG |
| 解像度 | 600×555px（元画像1303×1206pxからリサイズ） |
| 品質 | quality=75 |
...
| 犬関連（危険） | 赤系 | #e53935 |
| UI文字 | ダークグレー | #37474f |
| UIベース | 白系 | #fff / rgba(255,255,255,0.85) |
...
```javascript
// assets/sprites.png をインポート
import spriteUrl from './assets/sprites.png';
import bgUrl from './assets/bg.jpg';
import logoUrl from './assets/logo.png';
import dogBarkUrl from './assets/dog-bark.png';
